...
{
  LOG("[Hfp] %s, BatteryInfo.level(): %f", __FUNCTION__, aBatteryInfo.level());
...
BluetoothHfpManager::NotifyStatusChanged(const nsAString& aType)
{
  ...
  if (type.EqualsLiteral("bluetooth-hfp-status-changed")) {
    v = IsConnected();
    LOG("[Hfp] %s, address: %s, connected: %d", __FUNCTION__, NS_ConvertUTF16toUTF8(mDeviceAddress).get(), IsConnected());
  } else if (type.EqualsLiteral("bluetooth-sco-status-changed")) {
    v = IsScoConnected();
    LOG("[Hfp] %s, address: %s, connected: %d", __FUNCTION__, NS_ConvertUTF16toUTF8(mDeviceAddress).get(), IsScoConnected());
  } else {
...
{
  LOG("[Hfp] %s, command: %s", __FUNCTION__, NS_ConvertUTF16toUTF8(aCommand).get());
  nsString type, name;
  BluetoothValue v;
  InfallibleTArray<BluetoothNamedValue> parameters;
...
{
  LOG("[Hfp] %s, status: %d", __FUNCTION__, aStatus);
  MOZ_ASSERT(NS_IsMainThread());
...
BluetoothHfpManager::HandleVolumeChanged(const nsAString& aData)
{
  ...
  LOG("[Hfp] %s, volume: %d", __FUNCTION__, value.toNumber());
...
BluetoothHfpManager::HandleCallStateChanged(uint32_t aCallIndex,
                                            ...
                                            bool aSend)
{
  LOG("[Hfp] %s, (%d, %d, %s, %d)", __FUNCTION__, aCallIndex, aCallState, NS_ConvertUTF16toUTF8(aNumber).get(), aSend);
  if (!IsConnected()) {
...
{
  LOG("[Hfp] %s(%s)", __FUNCTION__, NS_ConvertUTF16toUTF8(aDeviceAddress).get());
  MOZ_ASSERT(NS_IsMainThread());
