                ...
                void *aParam)
{
  ...
  nsString address =
    GetAddressFromObjectPath(NS_ConvertUTF8toUTF16(objectPath));
  delete [] objectPath;
  LOG("[B] %s, aIsConnected: %d, address: %s", __FUNCTION__, aIsConnected, NS_ConvertUTF16toUTF8(address).get());
...
BluetoothDBusService::StopInternal()
{
  ...
  if (!sAdapterPath.IsEmpty()) {
    LOG("[B] sAdapterPath is not empty");
                                      ...
                                      const nsAString& aMessage)
{
  ...
  if (aMessage.EqualsLiteral("Connect")) {
    ...
    NS_WARNING("Unknown sink message");
    return NS_ERROR_FAILURE;
  }
...
public:
  ...
  NS_IMETHOD Run()
  {
    LOG("[B] BluetoothDevicePropertiesRunnable::Run");
    MOZ_ASSERT(!NS_IsMainThread());
...
public:
  ...
  Run()
  {
    LOG("[B] BluetoothPairedDevicePropertiesRunnable::Run");
    MOZ_ASSERT(!NS_IsMainThread());
    DBusError err;
    dbus_error_init(&err);
...
public:
  ...
  Run()
  {
    LOG("[B] [mt] OnGetServiceChannelRunnable::Run");
    MOZ_ASSERT(NS_IsMainThread());
...
public:
  ...
  Run()
  {
    LOG("[B] [ct] GetDeviceChannelRunnable::Run");
    MOZ_ASSERT(!NS_IsMainThread());
